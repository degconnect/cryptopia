...
class Cryptopia(object):
    ...
    def api_query(self, feature_requested, get_parameters=None, post_parameters=None):
        ...
        if feature_requested in self.private:
            url = BASE_URL + feature_requested
            ...
            if "Nonce has already been used for this request." in req.text:
                time.sleep(1)
                headers = self.secure_headers(url=url, post_data=post_data)
                req = requests.post(url, data=post_data, headers=headers)
                try:
            ...
            url = BASE_URL + feature_requested + "/" + '/'.join(i for i in get_parameters.values())
    ...
    def get_balance(self, currency):
        ...
        :return {"CurrencyId":699,"Symbol":"INN","Total":40.33564582,"Available":9.99977440,
                "Unconfirmed":0.00000000,"HeldForTrades":30.33587142,"PendingWithdraw":0.00000000,
                "Address":null,"Status":"OK","StatusMessage":null,"BaseAddress":""}
        """
    ...
    def secure_headers(self, url, post_data):
        ...
        sign = base64.b64encode(hmac.new(base64.b64decode(self.secret), signature, hashlib.sha256).digest())
